...
{
    public partial class frmColor : frmAddItems, IUpdatableForms
    {
        private readonly IUpdatableForms _updatableForms;
        public int ListId = 0;
        int UserId_ = PublicClass.UserId;

        public frmColor(IUpdatableForms updatableForms)
        {
            InitializeComponent();
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                if (PublicClass.FindEmptyControls(txtName, ResourceCode.T164))
                    return;
                using (var db = new DBcontextModel())
                {
                    ...
                    if (ListId == 0)
                    {
                        int cont = db.Color_s.Count(c => c.Name == txtName.Text);
                        if (cont > 0)
                        {
                    ...
                    {
                        int cont = db.Color_s.Count(c => c.Name == txtName.Text & c.Id != ListId);
                        if (cont > 0)
                        {
                            PublicClass.ErrorMesseg(ResourceCode.T163); return;
                        }
                    ...
                    var userRepo = new Repository<Entity.Color.Color_>(db);
                    if (userRepo.SaveOrUpdate(new Entity.Color.Color_ { Id = ListId, Name = txtName.Text }, ListId))
                    {
